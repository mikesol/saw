...
#(define (ms-namespace-make-coord x-value y-value)
 "Make a coordinate pair from @var{x-valye} and @var{y-value}."
 (cons x-value y-value))

#(define (ms-namespace-coord+ coord1 coord2)
...
#(define (ms-namespace-coord* scalar coord)
 "Multiply each component of @var{coord} by @var{scalar}."
 (cons (* (car coord) scalar)
       (* (cdr coord) scalar)))

#(define (ms-namespace-make-bezier point-0 point-1 point-2 point-3)
 "Create a cubic bezier from the four control points."
 (list point-0 point-1 point-2 point-3))

#(define (ms-namespace-interpolated-control-points control-points split-value)
...
set of control points that is one degree less than @var{control-points}."
  (if (null? (cdr control-points))
      ...
      (let ((first (car control-points))
       ...
       (cons* (ms-namespace-coord+ first (ms-namespace-coord* split-value (ms-namespace-coord- second first)))
             (ms-namespace-interpolated-control-points
              (cdr control-points)
              split-value)))))

#(define (ms-namespace-split-bezier bezier split-value)
...
element is the RHS spline."
   (let* ((quad-points (ms-namespace-interpolated-control-points
                       bezier
                       split-value))
          (lin-points (ms-namespace-interpolated-control-points
                       quad-points
                       split-value))
          (const-point (ms-namespace-interpolated-control-points
                        lin-points
                        split-value))
...
#(define (ms-namespace-multi-split-bezier bezier start-t split-list)
...
Returns a list of @var{(1+ (length split-list))} beziers."
  (let* ((bezier-split (ms-namespace-split-bezier bezier
    ...
    (if (null? (cdr split-list))
        bezier-split
        (cons* left-bezier
               (ms-namespace-multi-split-bezier right-bezier
                                   (car split-list)
                                   (cdr split-list))))))


#(define (ms-namespace-bezier-sandwich-list top-bezier bottom-bezier)
